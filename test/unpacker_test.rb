...
class UnpackerTest < Test::Unit::TestCase
  ...
  def test_uint8
    assert_equal(0,          @module.unpack(StringIO.new("\xCC\x00")))
    assert_equal(2 ** 8 - 1, @module.unpack(StringIO.new("\xCC\xFF")))
  end

  def test_uint16
    assert_equal(0,           @module.unpack(StringIO.new("\xCD\x00\x00")))
    assert_equal(1,           @module.unpack(StringIO.new("\xCD\x00\x01")))
    assert_equal(2 ** 16 - 1, @module.unpack(StringIO.new("\xCD\xFF\xFF")))
  end

  def test_uint32
    assert_equal(0,           @module.unpack(StringIO.new("\xCE\x00\x00\x00\x00")))
    assert_equal(1,           @module.unpack(StringIO.new("\xCE\x00\x00\x00\x01")))
    assert_equal(2 ** 32 - 1, @module.unpack(StringIO.new("\xCE\xFF\xFF\xFF\xFF")))
  end

  def test_uint64
    assert_equal(0,           @module.unpack(StringIO.new("\xCF\x00\x00\x00\x00\x00\x00\x00\x00")))
    assert_equal(1,           @module.unpack(StringIO.new("\xCF\x00\x00\x00\x00\x00\x00\x00\x01")))
    assert_equal(2 ** 64 - 1, @module.unpack(StringIO.new("\xCF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF")))
  end

  def test_int8
    assert_equal(0,          @module.unpack(StringIO.new("\xD0\x00")))
    assert_equal(2 ** 7 - 1, @module.unpack(StringIO.new("\xD0\x7F")))
    assert_equal(-1,         @module.unpack(StringIO.new("\xD0\xFF")))
    assert_equal(-(2 ** 7),  @module.unpack(StringIO.new("\xD0\x80")))
  end
  ...
  def test_double
    assert_equal(0.0,  @module.unpack(StringIO.new("\xCB\x00\x00\x00\x00\x00\x00\x00\x00")))
